...
function prepareAioncore(options) {
  ...
  // Idempotent: a matching aioncore already staged (a previous build or a
  // CI-cache restore) — skip the large download. Version- and platform-keyed
  // so a bumped backend, an arch change or an interrupted extraction always
  // re-downloads instead of reusing a stale binary.
  const manifestPath = path.join(targetDir, 'manifest.json');
  if (fs.existsSync(manifestPath)) {
    try {
      const existing = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
      const expectedVersion = actionsRunId ? `actions-run-${actionsRunId}` : tag;
      if (
        ...
        console.log(`  ✓ Bundled aioncore ${expectedVersion} already staged for ${runtimeKey}, skipping download`);
        return { prepared: true, dir: targetDir, sourceType: 'skip' };
      }
    } catch {
      // Corrupt/unreadable manifest — rebuild below.
    }
  }
