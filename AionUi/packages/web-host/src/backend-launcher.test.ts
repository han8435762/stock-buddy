...
describe('buildSpawnEnv', () => {
  ...
  it('exports bundled Python even when system directory overrides are omitted', () => {
    vi.mocked(readFileSync).mockImplementation(() => {
      throw new Error('no bundled CLI manifest');
    });
    vi.mocked(existsSync).mockReturnValue(true);

    const env = buildSpawnEnv(undefined, '/resources');
    const pythonRoot = join('/resources', 'bundled-python', `${process.platform}-${process.arch}`);
    const executable =
      process.platform === 'win32' ? join(pythonRoot, 'python.exe') : join(pythonRoot, 'bin', 'python3');

    expect(env.AIONUI_PYTHON_EXECUTABLE).toBe(executable);
    expect(env.PYTHONHOME).toBe(pythonRoot);
  });
});

describe('buildBundledCliPathEnv', () => {
  beforeEach(() => {
    vi.mocked(readFileSync).mockReset();
    vi.mocked(existsSync).mockReset();
  });

  it('returns {} when resourcesPath is missing', () => {
    expect(buildBundledCliPathEnv(undefined, '/usr/bin', 'win32', 'x64')).toEqual({});
  });

  it('returns {} when the managed-resources manifest is unreadable', () => {
    vi.mocked(readFileSync).mockImplementation(() => {
      throw new Error('ENOENT');
    });
    expect(buildBundledCliPathEnv('/resources', '/usr/bin', 'win32', 'x64')).toEqual({});
  });

  it('returns {} when the manifest declares no clis', () => {
    vi.mocked(readFileSync).mockReturnValue(JSON.stringify({ schemaVersion: 2, clis: [] }));
    expect(buildBundledCliPathEnv('/resources', '/usr/bin', 'win32', 'x64')).toEqual({});
  });

  it('appends each bundled CLI dir after the inherited PATH', () => {
    vi.mocked(readFileSync).mockReturnValue(
      JSON.stringify({
        schemaVersion: 2,
        clis: [
          { name: 'claude', root: 'cli/claude/2.1.215/win32-x64', executable: 'claude' },
          { name: 'codex', root: 'cli/codex/0.144.6/win32-x64', executable: 'vendor/x86_64-pc-windows-msvc/bin/codex' },
        ],
      })
    );
    vi.mocked(existsSync).mockReturnValue(true);

    const env = buildBundledCliPathEnv('/resources', '/usr/bin:/bin', 'win32', 'x64');

    const managed = join('/resources', 'bundled-aioncore', 'win32-x64', 'managed-resources');
    const claudeDir = join(managed, 'cli/claude/2.1.215/win32-x64');
    const codexDir = join(managed, 'cli/codex/0.144.6/win32-x64', 'vendor/x86_64-pc-windows-msvc/bin');
    expect(env.PATH).toBe(`${claudeDir}${delimiter}${codexDir}${delimiter}/usr/bin:/bin`);
  });

  it('skips CLI dirs that do not exist on disk', () => {
    vi.mocked(readFileSync).mockReturnValue(
      JSON.stringify({
        clis: [
          { name: 'claude', root: 'cli/claude/2.1.215/win32-x64', executable: 'claude' },
          { name: 'codex', root: 'cli/codex/0.144.6/win32-x64', executable: 'codex' },
        ],
      })
    );
    vi.mocked(existsSync).mockImplementation((p: string) => String(p).includes('claude'));

    const env = buildBundledCliPathEnv('/resources', '/sys', 'win32', 'x64');
    ...
    const claudeDir = join(
      ...
      'win32-x64',
      'managed-resources',
      'cli/claude/2.1.215/win32-x64'
    );
    expect(env.PATH).toBe(`${claudeDir}${delimiter}/sys`);
  });

  it('skips clis missing root or executable', () => {
    vi.mocked(readFileSync).mockReturnValue(
      JSON.stringify({
        clis: [
          { name: 'broken', root: 'cli/broken/1.0.0/win32-x64' },
          { name: 'also-broken', executable: 'nope' },
          { name: 'claude', root: 'cli/claude/2.1.215/win32-x64', executable: 'claude' },
        ],
      })
    );
    ...
    const env = buildBundledCliPathEnv('/resources', undefined, 'win32', 'x64');
    ...
    expect(env.PATH).toBe(claudeDir);
  });
});

describe('buildBundledPythonEnv', () => {
  beforeEach(() => {
    vi.mocked(existsSync).mockReset();
  });

  it('exposes the bundled Windows interpreter to backend tools on a clean machine', () => {
    vi.mocked(existsSync).mockReturnValue(true);

    const env = buildBundledPythonEnv('/resources', '', 'win32', 'x64');
    const pythonRoot = join('/resources', 'bundled-python', 'win32-x64');

    expect(env.PATH).toBe(pythonRoot);
    expect(env.PYTHONHOME).toBe(pythonRoot);
    expect(env.AIONUI_PYTHON_EXECUTABLE).toBe(join(pythonRoot, 'python.exe'));
  });

  it('preserves the inherited PATH after the bundled interpreter directory', () => {
    vi.mocked(existsSync).mockReturnValue(true);

    const env = buildBundledPythonEnv('/resources', '/system/bin', 'linux', 'arm64');
    const pythonRoot = join('/resources', 'bundled-python', 'linux-arm64');

    expect(env.PATH).toBe(`${join(pythonRoot, 'bin')}${delimiter}/system/bin`);
    expect(env.PYTHONHOME).toBe(pythonRoot);
    expect(env.AIONUI_PYTHON_EXECUTABLE).toBe(join(pythonRoot, 'bin', 'python3'));
  });

  it('does not modify the backend environment when the bundled interpreter is missing', () => {
    vi.mocked(existsSync).mockReturnValue(false);

    expect(buildBundledPythonEnv('/resources', '/system/bin', 'win32', 'x64')).toEqual({});
  });
});
