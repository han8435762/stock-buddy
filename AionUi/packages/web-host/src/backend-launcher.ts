...
/**
 * Append the bundled managed-CLI executable directories to PATH so aioncore's
 * agent detection can find the CLI binaries shipped inside the app.
 *
 * Packaged builds bundle `claude` / `codex` binaries under
 * `resources/bundled-aioncore/<runtimeKey>/managed-resources/`, but aioncore
 * locates ACP agents by resolving the binary name on the system PATH — it
 * reports `primary binary 'claude' not on $PATH` when absent. On machines that
 * never installed these CLIs (e.g. a fresh Windows install) the bundled
 * binaries stay invisible, so the built-in agents show as `missing` and never
 * appear in the assistant list. Appending each CLI's executable directory to
 * PATH — after the inherited entries so a system-installed CLI still wins —
 * exposes the bundled binaries without a manual install.
 *
 * Paths come from the managed-resources manifest generated by `aioncore
 * prepare-managed-resources`, so they stay in sync with whatever version was
 * bundled at build time.
 */
export function buildBundledCliPathEnv(
  ...
  const managedRoot = join(resourcesPath, 'bundled-aioncore', runtimeKey, 'managed-resources');
  ...
  if (!manifest || typeof manifest !== 'object' || !Array.isArray((manifest as { clis?: unknown }).clis)) {
    return {};
  }

  const cliDirs: string[] = [];
  for (const cli of (manifest as { clis: Array<{ root?: unknown; executable?: unknown }> }).clis) {
    if (!cli || typeof cli !== 'object') continue;
    const root = cli.root;
    const executable = cli.executable;
    if (typeof root !== 'string' || typeof executable !== 'string') continue;
    // `executable` is a posix-style path relative to `root`; dirname of a bare
    // name (e.g. `claude`) is '.', so the CLI dir for those is its own root.
    const executableDir = posix.dirname(executable);
    const cliDir = join(managedRoot, root, executableDir);
    if (existsSync(cliDir)) cliDirs.push(cliDir);
  }
  if (cliDirs.length === 0) return {};

  const mergedPath = [...cliDirs, existingPath ?? ''].filter(Boolean).join(delimiter);
  return { PATH: mergedPath };
}

/**
 * Expose the portable Python runtime shipped in `extraResources` to aioncore
 * and every shell/tool process it starts. StockBuddy's direct data provider
 * uses an absolute interpreter path, but agent-authored PowerShell commands
 * resolve `python` through PATH and therefore need the same runtime exported
 * at the backend process boundary.
 */
export function buildBundledPythonEnv(
  ...
  const pythonRoot = join(resourcesPath, 'bundled-python', `${platform}-${arch}`);
  const executable = platform === 'win32' ? join(pythonRoot, 'python.exe') : join(pythonRoot, 'bin', 'python3');
  if (!existsSync(executable)) return {};

  const binDir = platform === 'win32' ? pythonRoot : join(pythonRoot, 'bin');
  const mergedPath = [binDir, existingPath ?? ''].filter(Boolean).join(delimiter);
  return {
    PATH: mergedPath,
    PYTHONHOME: pythonRoot,
    AIONUI_PYTHON_EXECUTABLE: executable,
...
export function buildSpawnEnv(
  ...
  const cliEnv = buildBundledCliPathEnv(resourcesPath, process.env.PATH);
  const pythonEnv = buildBundledPythonEnv(resourcesPath, cliEnv.PATH ?? process.env.PATH);
  return {
    ...process.env,
    ...(dirs
      ? {
  ...
  private async attemptStart(
    ...
    try {
      this.childProcess = spawn(binaryPath, args, {
        ...
        env: buildSpawnEnv(dirs, this.appMeta.resourcesPath),
