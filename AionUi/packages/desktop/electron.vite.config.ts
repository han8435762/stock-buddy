...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    renderer: {
      ...
      appType: 'mpa',
      // The repository root has a separate site's Tailwind PostCSS config.
      // Keep it from being discovered by the desktop renderer, which uses
      // UnoCSS and installs dependencies from the AionUi workspace only.
      css: {
        postcss: { plugins: [] },
      },
      server: {
