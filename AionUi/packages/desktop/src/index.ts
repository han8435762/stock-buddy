...
function markBackendReady(backendPort: number, source: string): void {
  ...
  // Prune the builtin-skill corpus this product does not expose. aioncore
  // materializes every builtin skill on first run / version upgrade, so this
  // runs once per process after the backend is ready.
  void pruneUnusedBuiltinSkillsOnReady();
  // Import the stock-research skills shipped with the stock-buddy repo into the
  // user skill library (only the ones missing — user edits are preserved).
  void importStockbuddySkillsOnReady();
}

/** Fire-and-forget cleanup of the unused builtin-skill directories. */
function pruneUnusedBuiltinSkillsOnReady(): void {
  void (async () => {
    try {
      const { getDataPath } = await import('./process/utils/utils');
      const { pruneUnusedBuiltinSkills } = await import('./process/utils/pruneUnusedBuiltinSkills');
      await pruneUnusedBuiltinSkills(getDataPath());
    } catch (error) {
      console.error('[AionUi] Failed to prune unused builtin skills:', error);
    }
  })();
}

/** Fire-and-forget import of the stockbuddy skill corpus. */
function importStockbuddySkillsOnReady(): void {
  void (async () => {
    try {
      const { importStockbuddySkills } = await import('./process/utils/importStockbuddySkills');
