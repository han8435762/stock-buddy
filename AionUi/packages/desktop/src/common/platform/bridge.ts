...
export const subscribe = <Params = unknown, Data = unknown>(
  ...
  on(`subscribe-${name}`, (request) => {
    ...
    // `Promise.resolve().then(...)` so BOTH a synchronous throw and an async
    // rejection from the provider reach the same `.catch` — and, on failure,
    // the invoke is rejected on the other side instead of left pending forever.
    Promise.resolve()
      ...
      .catch((error: unknown) => {
        ...
        emit(`subscribe.error-${name}${request.id}`, serializeError(error));
      });
  });
...
export const invoke = <Data = unknown>(name: string, data?: unknown): Promise<Data> => {
  ...
  const errorName = `subscribe.error-${name}${id}`;
  ...
  return new Promise<Data>((resolve, reject) => {
    ...
    const disposeError = on(errorName, (payload) => {
      disposeSuccess();
      disposeError();
      reject(deserializeError(payload));
    });
    emit(`subscribe-${name}`, { id, data });
  });
};

/** Errors must cross a JSON transport (Electron IPC / WebSocket), so they are
 *  reduced to a plain, marked payload before being emitted. */
const serializeError = (error: unknown): Record<string, unknown> => {
  ...
  if (typeof error === 'object' && error !== null) {
    return { __bridge_error: true, ...(error as Record<string, unknown>) };
  }
  return { __bridge_error: true, message: String(error) };
};

const deserializeError = (payload: unknown): Error => {
  if (typeof payload === 'object' && payload !== null && (payload as Record<string, unknown>).__bridge_error) {
    const { name, message, stack, ...rest } = payload as Record<string, unknown>;
    const error = new Error(typeof message === 'string' ? message : 'Bridge provider error');
    if (typeof name === 'string') error.name = name;
    if (typeof stack === 'string') error.stack = stack;
    Object.assign(error, rest);
    return error;
  }
  return new Error('Bridge provider error');
};
