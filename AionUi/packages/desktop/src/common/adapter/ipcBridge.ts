...
import type {
  ClaudeAuthStatus,
  ClaudeCliInstalledResult,
  ClaudeCodeEnv,
  WriteClaudeCodeEnvResult,
} from '../types/claudeCodeConfig';
import type {
...
/**
 * Builtin skills kept visible in the UI. StockBuddy ships a large builtin
 * corpus (officecli-*, recruiters, ppt themes, …) that this product does not
 * use; only cron / officecli / skill-creator are surfaced. Custom and extension
 * skills are always kept. Filtering `listAvailableSkills` here keeps every
 * consumer free of skills the product no longer exposes.
 */
const SKILL_VISIBLE_BUILTIN = new Set(['cron', 'officecli', 'skill-creator']);
...
export const fs = {
  ...
  listAvailableSkills: withResponseMap(
    httpGet<
      ...
      void
    >('/api/skills'),
    (skills) => skills.filter((skill) => skill.source !== 'builtin' || SKILL_VISIBLE_BUILTIN.has(skill.name))
  ),
  ...
  writeSkillFile: bridge.buildProvider<void, { skill_location: string; relative_path: string; content: string }>(
    'skills.files.write'
  ),
  getSkillDefaultFile: bridge.buildProvider<string, { skill_location: string; relative_path: string }>(
...
// ---------------------------------------------------------------------------
// PDF generation — Electron-native (marked + hidden BrowserWindow printToPDF)
// ---------------------------------------------------------------------------

export const pdf = {
  /** Convert markdown to a PDF, returned as a base64 string (null on failure). */
  markdownToPdf: bridge.buildProvider<string | null, { markdown: string }>('pdf.markdown-to-pdf'),
};
...
// ---------------------------------------------------------------------------
// Claude Code model config — Electron-only IPC (reads/writes ~/.claude/settings.json)
// ---------------------------------------------------------------------------

export const claudeCode = {
  getEnv: bridge.buildProvider<ClaudeCodeEnv, void>('claude-code:get-env'),
  setEnv: bridge.buildProvider<WriteClaudeCodeEnvResult, { env: ClaudeCodeEnv }>('claude-code:set-env'),
  clearEnv: bridge.buildProvider<WriteClaudeCodeEnvResult, void>('claude-code:clear-env'),
  getAuthStatus: bridge.buildProvider<ClaudeAuthStatus, void>('claude-code:get-auth-status'),
  checkCliInstalled: bridge.buildProvider<ClaudeCliInstalledResult, void>('claude-code:check-cli-installed'),
};
...
export interface ICreateConversationParams {
  ...
  extra: {
    ...
    /** StockBuddy company binding written when a conversation is created in a
     *  company folder; drives "recent research" sidebar grouping. */
    company_id?: string;
    company_name?: string;
    default_files?: string[];
    /** Exact ChatFileRef[] attached at send time (uploads + selected company
     *  materials). Persisted so reopening a research conversation can prefill
     *  the sendbox with the same attachments. */
    attached_refs?: ChatFileRef[];
    cli_path?: string;
    gateway?: {
      host?: string;
      port?: number;
...
/**
 * StockBuddy local company library — served by the main-process local service
 * (see process/services/stockbuddy), not by the aioncore backend.
 */
export const stockbuddy = {
  getRootDir: bridge.buildProvider<string, void>('stockbuddy.root-dir'),
  changeRootDir: bridge.buildProvider<string, { directory: string }>('stockbuddy.root-dir.change'),
  rootDirChanged: bridge.buildEmitter<{ previousRoot: string; nextRoot: string }>('stockbuddy.root-dir.changed'),
  getResearchWorkspaceDir: bridge.buildProvider<string, void>('stockbuddy.research-workspace-dir'),
  searchCompanies: bridge.buildProvider<CompanySearchResult[], { query: string }>('stockbuddy.search-companies'),
  listCompanies: bridge.buildProvider<CompanyMetadata[], void>('stockbuddy.companies.list'),
  getCompany: bridge.buildProvider<CompanyMetadata | null, { code: string }>('stockbuddy.company.get'),
  getCompanyDir: bridge.buildProvider<string | null, { code: string }>('stockbuddy.company.dir'),
  createCompany: bridge.buildProvider<CompanyMetadata, CreateCompanyInput>('stockbuddy.company.create'),
  removeCompany: bridge.buildProvider<void, { code: string; deleteFolder?: boolean }>('stockbuddy.company.remove'),
  openCompanyFolder: bridge.buildProvider<void, { code: string }>('stockbuddy.company.open-folder'),
  getManifest: bridge.buildProvider<CompanyManifest | null, { code: string }>('stockbuddy.manifest.get'),
  listMaterials: bridge.buildProvider<Material[], { code: string }>('stockbuddy.materials.list'),
  getMaterialTree: bridge.buildProvider<MaterialTreeNode[], { code: string }>('stockbuddy.materials.tree'),
  addMaterial: bridge.buildProvider<Material, { code: string; material: Material }>('stockbuddy.materials.add'),
  updateMaterial: bridge.buildProvider<Material, { code: string; id: string; patch: Partial<Material> }>(
    'stockbuddy.materials.update'
  ),
  removeMaterial: bridge.buildProvider<void, { code: string; id: string }>('stockbuddy.materials.remove'),
  deleteMaterialFile: bridge.buildProvider<void, { code: string; path: string }>('stockbuddy.materials.delete-file'),
  listJobs: bridge.buildProvider<UpdateJob[], void>('stockbuddy.jobs.list'),
  getJob: bridge.buildProvider<UpdateJob | null, { id: string }>('stockbuddy.job.get'),
  createJob: bridge.buildProvider<UpdateJob, { companyCode: string }>('stockbuddy.job.create'),
  runJob: bridge.buildProvider<UpdateJob, { id: string }>('stockbuddy.job.run'),
  pauseJob: bridge.buildProvider<UpdateJob, { id: string }>('stockbuddy.job.pause'),
  resumeJob: bridge.buildProvider<UpdateJob, { id: string }>('stockbuddy.job.resume'),
  cancelJob: bridge.buildProvider<UpdateJob, { id: string }>('stockbuddy.job.cancel'),
  removeJob: bridge.buildProvider<void, { id: string }>('stockbuddy.job.remove'),
  jobUpdated: bridge.buildEmitter<UpdateJob>('stockbuddy.job.updated'),
  importFiles: bridge.buildProvider<Material[], { code: string; paths: string[] }>('stockbuddy.import-files'),
  listSchedules: bridge.buildProvider<UpdateSchedule[], void>('stockbuddy.schedules.list'),
  createSchedule: bridge.buildProvider<UpdateSchedule, { companyCode: string; frequencyMinutes: number }>(
    'stockbuddy.schedules.create'
  ),
  updateSchedule: bridge.buildProvider<UpdateSchedule, { id: string; patch: Partial<UpdateSchedule> }>(
    'stockbuddy.schedules.update'
  ),
  removeSchedule: bridge.buildProvider<void, { id: string }>('stockbuddy.schedules.remove'),
  runScheduleNow: bridge.buildProvider<void, { id: string }>('stockbuddy.schedules.run-now'),
};
