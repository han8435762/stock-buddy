...
const DEFAULT_REPO = 'han8435762/stock-buddy';
const DEFAULT_USER_AGENT = 'StockBuddy';
...
const mapGitHubRelease = (release: GitHubReleaseApi): UpdateReleaseInfo | null => {
  ...
  if (!version || release.draft) return null;
...
const selectLatestGitHubRelease = (
  ...
  return candidates.toSorted((a, b) => semver.rcompare(a.version, b.version))[0] ?? null;
};
...
const sanitizeFileName = (name: string): string => {
  ...
  // Avoid empty names.
  return base || `StockBuddy-update-${Date.now()}`;
};
  ...
  ipcBridge.update.check.provider(
    async (params): Promise<{ success: boolean; data?: UpdateCheckResult; msg?: string }> => {
      try {
        ...
        // StockBuddy GitHub Release version. CI must inject a
        // prerelease semver into `package.json#version` for dev builds (e.g. `1.7.2-dev.1234+sha.abcdef0`)
        // so semver ordering holds.
        //
        // 中文：版本号说明
        // 更新比较严格使用 semver：`app.getVersion()`（应用自身版本号）对比 StockBuddy GitHub Release 版本。
        // 若要 dev/预发布版本更新可靠生效，需要 CI 在 dev 构建时把 `package.json#version`
        // 注入为带 prerelease 的 semver（如 `1.7.2-dev.1234+sha.abcdef0`），以保证比较顺序正确。

        // StockBuddy GitHub Releases are the single source of truth for update
        // metadata, release notes and downloadable installers.
        const releases = await fetchGitHubReleases(repo);
        const latest = selectLatestGitHubRelease(releases, Boolean(params?.includePrerelease));
