...
export const createSkillFileService = () => ({
  ...
  /**
   * Overwrite an existing file inside the skill directory. The target must
   * already exist — this edits existing files, it never creates new ones.
   */
  async write(skillLocation: string, relativePath: string, content: string): Promise<void> {
    const root = await resolveSkillRoot(skillLocation);
    const target = await resolveExistingEntry(root, relativePath);
