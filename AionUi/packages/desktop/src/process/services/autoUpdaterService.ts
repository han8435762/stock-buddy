...
import { buildStockBuddyFeedOptions } from './updateFeed';
...
class AutoUpdaterService extends EventEmitter {
  ...
  constructor() {
    ...
    const stockBuddyFeedOptions = buildStockBuddyFeedOptions();
    ...
    autoUpdater.setFeedURL(stockBuddyFeedOptions);
    log.info('Update feed set to StockBuddy GitHub releases');
    log.debug('[auto-update] StockBuddy feed configured', {
      provider: stockBuddyFeedOptions.provider,
      owner: stockBuddyFeedOptions.owner,
      repo: stockBuddyFeedOptions.repo,
  ...
  async checkForUpdates(): Promise<{ success: boolean; updateInfo?: UpdateInfo; error?: string }> {
    try {
      ...
      if (this._allowPrerelease) {
        ...
        log.debug('[auto-update] Stable GitHub feed skipped because prerelease mode is handled by the releases API');
        return { success: true };
      }
      ...
      if (!result.isUpdateAvailable) {
        log.debug('[auto-update] no update available from StockBuddy GitHub feed', {
          version: result.updateInfo.version,
        });
        return { success: true };
      }
      log.debug('[auto-update] update available from StockBuddy GitHub feed', {
        version: result.updateInfo.version,
