...
import GuidMaterialScope from './components/GuidMaterialScope';
import GuidModelSelector from './components/GuidModelSelector';
import { useGuidAssistantSelection } from './hooks/useGuidAssistantSelection';
import { useGuidInput } from './hooks/useGuidInput';
import { useGuidModelSelection } from './hooks/useGuidModelSelection';
import { useGuidSend } from './hooks/useGuidSend';
import { useMaterialAttachmentSync } from './hooks/useMaterialAttachmentSync';
import { useResearchMaterialScope } from './hooks/useResearchMaterialScope';
import { parseCompanyDir } from './utils/companyFolder';
...
type GuidNavigationState = {
  ...
  workspace?: string;
  preferredAssistantBackend?: string;
  companyResearch?: boolean;
  /** Manifest material id to pre-select as the research material scope. */
  preselectMaterialId?: string;
  /** Library file relative path to pre-select as the research material scope. */
...
const GuidPage: React.FC = () => {
  ...
  const [claudeUnavailableModal, setClaudeUnavailableModal] = useState(false);
  ...
  const isCompanyResearch = navState?.companyResearch === true;
  ...
  const agentSelection = useGuidAssistantSelection({
    ...
    preferredAssistantBackend: navState?.preferredAssistantBackend,
  ...
  // StockBuddy: the selected company derives from the workspace folder
  // (folder names are `${code}_${name}`); it drives the material-scope selector.
  const selectedCompany = useMemo(() => (guidInput.dir ? parseCompanyDir(guidInput.dir) : null), [guidInput.dir]);
  const companyId = selectedCompany?.companyId ?? '';
  const materialScope = useResearchMaterialScope(companyId, navState?.preselectMaterialId, navState?.preselectFile);
  // Selected materials flow into the same attachment list as uploaded/picked
  // files (`guidInput.files`) and appear as removable chips in the input card.
  const attachmentSync = useMaterialAttachmentSync({
    companyId,
    scope: materialScope.scope,
    selectedPaths: materialScope.selectedPaths,
    removeFileByPath: materialScope.removeFileByPath,
  ...
  const send = useGuidSend({
    ...
    // StockBuddy material scope (selected material refs already live in
    // `guidInput.files` via useMaterialAttachmentSync, so only metadata is sent)
    researchScope: materialScope.scope,
    selectedMaterialIds: materialScope.selectedMaterialIds,
    companyResearch: isCompanyResearch,
  ...
  // StockBuddy: guard Claude Code sends — surface a guide to model settings
  // before creating a session that would fail when the CLI or provider is gone.
  const guardedSend = useCallback(() => {
    if (agentSelection.selectedAssistantBackend === 'claude') {
      void checkClaudeCodeAvailable()
        .then((available) => {
          if (available) {
            send.sendMessageHandler();
          } else {
            setClaudeUnavailableModal(true);
          }
        })
        .catch(() => send.sendMessageHandler());
      return;
    }
    send.sendMessageHandler();
  }, [agentSelection.selectedAssistantBackend, send.sendMessageHandler]);
  ...
  const handleInputKeyDown = useCallback(
    ...
    [guidInput.input, guardedSend, slashController]
  );
  ...
  const selectedAssistantPrompts = useMemo(() => {
    if (isCompanyResearch) {
      return [t('stockbuddy.newResearch.promptResearchCompany'), t('stockbuddy.newResearch.promptRecentAnnouncements')];
    }
  ...
  }, [isCompanyResearch, localeKey, selectedAssistantDetail, selectedAssistantRecord, selectedAssistantId, t]);

  const initializedCompanyResearchSkillsKeyRef = useRef<string | null>(null);
  useEffect(() => {
    if (!isCompanyResearch) {
      initializedCompanyResearchSkillsKeyRef.current = null;
      return;
    }
    if (!skillsLoaded || initializedCompanyResearchSkillsKeyRef.current === location.key) return;

    initializedCompanyResearchSkillsKeyRef.current = location.key;
    setGuidDisabledBuiltinSkills([]);
    setGuidEnabledSkills(allSkills.filter((skill) => !skill.isAuto).map((skill) => skill.name));
  }, [allSkills, isCompanyResearch, location.key, skillsLoaded]);

  const appliedCompanyResearchPermissionKeyRef = useRef<string | null>(null);
  useEffect(() => {
    if (!isCompanyResearch) {
      appliedCompanyResearchPermissionKeyRef.current = null;
      return;
    }
    if (agentSelection.selectedAssistantBackend !== 'claude') return;

    const availableModeIds = agentSelection.currentAgentModeOptions.map((mode) => mode.value);
    const signature = `${location.key}:${agentSelection.selectedAssistantBackend}:${availableModeIds.join(',')}`;
    if (appliedCompanyResearchPermissionKeyRef.current === signature) return;
    if (availableModeIds.length > 0 && !availableModeIds.includes('bypassPermissions')) return;

    appliedCompanyResearchPermissionKeyRef.current = signature;
    agentSelection.setSelectedMode('bypassPermissions', { persistPreference: false });
  }, [
  ...
  }, [isCompanyResearch, selectedAssistantDetail, selectedAssistantId]);
  ...
  useEffect(() => {
    ...
    const applyAssistantDefaults = async () => {
      ...
      const permissionMode = isCompanyResearch ? undefined : resolvedDefaults.permissionMode;
      if (permissionMode) {
        const availableModeIds = new Set(agentSelection.currentAgentModeOptions.map((mode) => mode.value));
        if (availableModeIds.size === 0 || availableModeIds.has(permissionMode)) {
          agentSelection.setSelectedMode(permissionMode, { persistPreference: false });
        } else {
  ...
  const materialScopeNode = (
    <GuidMaterialScope
      enabled={Boolean(companyId)}
      tree={materialScope.tree}
      disabledKeys={materialScope.disabledKeys}
      scope={materialScope.scope}
      onScopeChange={materialScope.setScope}
      selectedKeys={materialScope.selectedKeys}
      onSelectedKeysChange={materialScope.setSelectedKeys}
      selectedCount={materialScope.selectedPaths.length}
      onClearSelected={materialScope.clearSelected}
      modalOpen={materialScope.modalOpen}
      onModalOpenChange={materialScope.setModalOpen}
    />
  );
  ...
  const actionRowNode = (
    <GuidActionRow
      ...
      inlineCapabilities={isCompanyResearch}
      speechInputNode={
      ...
      <div ref={guidContainerRef} className={styles.guidContainer}>
        <div className={styles.guidLayout}>
          ...
          <GuidInputCard
            ...
            files={displayFilePaths}
            onRemoveFile={attachmentSync.handleRemoveFile}
            actionRow={actionRowNode}
            slashCommandMenu={slashCommandMenuNode}
            workspaceDir={guidInput.dir}
            onSelectWorkspace={(dir) => guidInput.setDir(dir)}
            onClearWorkspace={() => guidInput.setDir('')}
            workspaceTrailing={materialScopeNode}
          />
        ...
        <Modal
          title={t('stockbuddy.newResearch.claudeUnavailableTitle')}
          visible={claudeUnavailableModal}
          onCancel={() => setClaudeUnavailableModal(false)}
          footer={
            <div className='flex items-center justify-end gap-8px'>
              <Button
                onClick={() => {
                  setClaudeUnavailableModal(false);
                  send.sendMessageHandler();
                }}
              >
                {t('stockbuddy.newResearch.continueCreate')}
              </Button>
              <Button
                type='primary'
                onClick={() => {
                  setClaudeUnavailableModal(false);
                  void navigate('/settings/model?provider=deepseek');
                }}
              >
                {t('stockbuddy.newResearch.goConfigureModel')}
              </Button>
            </div>
          }
          ...
          alignCenter
        >
          <div className='text-14px text-t-primary'>{t('stockbuddy.newResearch.claudeUnavailableDesc')}</div>
        </Modal>
      </div>
    </ConfigProvider>
  );
