...
export const useGuidInput = ({ locationState }: UseGuidInputOptions): GuidInputResult => {
  ...
  // Read workspace from location.state (passed from tabs add button). A
  // `company:<code>` synthetic key (legacy sidebar company-group links) is
  // resolved to the real company folder path before being set.
  useEffect(() => {
    const workspace = locationState?.workspace;
    if (!workspace) return;
    if (workspace.startsWith('company:')) {
      const code = workspace.slice('company:'.length);
      let cancelled = false;
      ipcBridge.stockbuddy.getCompanyDir
        .invoke({ code })
        .then((dir) => {
          if (!cancelled && dir) setDir(dir);
        })
        .catch(() => {
          // unresolved synthetic key: leave dir empty
        });
      return () => {
        cancelled = true;
      };
