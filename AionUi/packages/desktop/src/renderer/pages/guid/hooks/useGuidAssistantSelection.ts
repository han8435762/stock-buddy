...
export function pickPreferredAssistantSelectionKey(assistants: Assistant[], preferredBackend?: string): string | null {
  const enabledAssistants = assistants.filter((assistant) => assistant.enabled !== false);
  const preferred = preferredBackend
    ? enabledAssistants.find((assistant) => assistantRuntimeKey(assistant) === preferredBackend)
    : undefined;
  return preferred?.id ?? pickDefaultAssistantSelectionKey(assistants);
}
...
export const useGuidAssistantSelection = ({
  ...
  useLayoutEffect(() => {
    ...
    if (resetAssistant || preferredAssistantBackend) {
      resetHandledRef.current = true;
      const fallbackId = preferredAssistantBackend
        ? pickPreferredAssistantSelectionKey(assistants, preferredAssistantBackend)
  ...
  }, [assistants, preferredAssistantBackend, preselectAssistantId, resetAssistant]);
  ...
  }, [assistants, preferredAssistantBackend, preselectAssistantId, resetAssistant, selectedAssistantIdState]);
