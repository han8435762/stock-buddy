...
type GuidActionRowProps = {
  ...
  /** Company research keeps capability pickers visible beside model/permission controls. */
...
const GuidActionRow: React.FC<GuidActionRowProps> = ({
  ...
  const menuContent = (
    ...
    >
      ...
      {!inlineCapabilities && allSkills.length > 0 && (
        <Menu.SubMenu
          key='skills'
          title={
            <div className='flex items-center gap-8px'>
      ...
      {!inlineCapabilities && mcpServers.length > 0 && (
        <Menu.SubMenu
          key='mcp'
          title={
            <div className='flex items-center gap-8px'>
  ...
  return (
    <div className={styles.actionRow}>
      ...
      <div className={styles.actionSubmit}>
        {!isMobile && inlineCapabilities ? (
          <div className={styles.inlineCapabilities}>
            <Dropdown
              trigger='click'
              droplist={<Menu className='min-w-240px'>{skillSelectionContent}</Menu>}
              onVisibleChange={(visible) => !visible && setSkillQuery('')}
            >
              <Button
                type='text'
                className={`${styles.inlineCapabilityButton} ${styles.guidCompactControl}`}
                icon={<Lightning theme='filled' size='14' fill={iconColors.secondary} />}
                data-testid='guid-inline-skills'
              >
                {t('settings.capabilitiesTab.skills')} ({activeSkillCount}/{allSkills.length})
              </Button>
            </Dropdown>
            <Dropdown
              trigger='click'
              droplist={<Menu className='min-w-240px'>{mcpSelectionContent}</Menu>}
              onVisibleChange={(visible) => !visible && setMcpQuery('')}
            >
              <Button
                type='text'
                className={`${styles.inlineCapabilityButton} ${styles.guidCompactControl}`}
                icon={<Shield theme='outline' size='14' fill={iconColors.secondary} />}
                data-testid='guid-inline-mcp'
              >
                {t('mcp.label')} ({activeMcpCount}/{mcpServers.length})
              </Button>
            </Dropdown>
