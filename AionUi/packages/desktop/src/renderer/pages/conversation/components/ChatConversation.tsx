...
import ResearchCitationPanel from '@/renderer/pages/stockbuddy/ResearchCitationPanel';
...
const AionrsConversationPanel: React.FC<{ conversation: AionrsConversation; sliderTitle: React.ReactNode }> = ({
  ...
  const isResearch = Boolean((conversation?.extra as { company_id?: string } | undefined)?.company_id);

  const chatLayoutProps = {
    title: conversation.name,
    siderTitle: sliderTitle,
    sider: isResearch ? (
      <ResearchCitationPanel conversation={conversation} />
    ) : (
...
const ChatConversation: React.FC<{
      ...
      case 'antigravity':
        return (
          <AcpChat
            ...
            initialAttachedRefs={(conversation.extra as { attached_refs?: ChatFileRef[] } | undefined)?.attached_refs}
          ></AcpChat>
        );
      default:
        return null;
  ...
  return (
    <ChatLayout
      ...
      siderTitle={sliderTitle}
      sider={
        isResearch ? <ResearchCitationPanel conversation={conversation} /> : <ChatSlider conversation={conversation} />
      }
