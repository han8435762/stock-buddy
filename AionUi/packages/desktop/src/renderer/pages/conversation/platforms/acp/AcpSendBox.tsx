...
// Conversations whose sendbox has already been prefilled from `attached_refs`.
// Module-level so a user's removal is respected across remounts within the app
// session (reopening a research conversation does not re-add removed chips).
const prefilledConversations = new Set<string>();
...
const AcpSendBox: React.FC<{
  ...
  /** Attachments persisted at creation (uploads + selected materials); prefills
   *  the sendbox once so follow-up turns keep the same file scope. */
  initialAttachedRefs?: ChatFileRef[];
}> = ({
  conversation_id,
  backend,
  session_mode,
  ...
  // Prefill the sendbox once with the conversation's persisted attachments
  // (uploads + selected research materials) so follow-up turns keep the same
  // file scope. Never touches a draft the user has already started (content,
  // `atPath` or `uploadFile` non-empty), and the module-level `prefilledConversations`
  // set keeps a user's explicit removal from being undone on remount.
  useEffect(() => {
    if (!initialAttachedRefs?.length) return;
    if (content || atPath.length > 0 || uploadFile.length > 0) return;
    if (prefilledConversations.has(conversation_id)) return;
    prefilledConversations.add(conversation_id);
    const { uploadFiles, atPath: restoredAtPath } = splitChatFileRefs(initialAttachedRefs);
    if (restoredAtPath.length > 0) setAtPath(restoredAtPath);
    if (uploadFiles.length > 0) setUploadFile(uploadFiles);
  }, [initialAttachedRefs, content, atPath, uploadFile, setAtPath, setUploadFile, conversation_id]);
