...
import { onReconnect, reveal, select } from './explorerStore';
...
export type ExplorerPreviewPayload = {
  ...
  metadata: {
    title: string;
    file_name: string;
    // Absolute backend-host path resolved from the explorer root's display path.
    // The bundled runtime exposes path-based preview endpoints.
    file_path: string;
    // Root passed to the backend sandbox checks for path-based file I/O.
    workspace: string;
    language: string;
    editable?: boolean;
  };
...
// Resolve the tree's `{pe_id, relative_path}` through the root display path.
// Text uses `/api/fs/read`, images/PDF use `/api/fs/image-base64`, and Office
// viewers receive the resolved path directly.
export const buildExplorerPreviewPayload = async (
  ...
  if (contentType === 'image') {
    content = (await ipcBridge.fs.getImageBase64.invoke({ path: filePath, workspace: rootPath })) || '';
  } else if (contentType === 'pdf') {
    content = (await ipcBridge.fs.getImageBase64.invoke({ path: filePath, workspace: rootPath })) || '';
  } else if (contentType === 'word' || contentType === 'excel' || contentType === 'ppt') {
    // Office viewers receive the absolute backend-host path below.
  } else {
    content = (await ipcBridge.fs.readFile.invoke({ path: filePath, workspace: rootPath })) || '';
  }
...
export const ExplorerContainer: React.FC<ExplorerContainerProps> = ({ projectId }) => {
  ...
  // Root migration keeps project ids stable and updates their folder records in
  // place. Refetch immediately so an already-open conversation drops the stale
  // display_path instead of requiring an application restart.
  useEffect(
    () =>
      ipcBridge.stockbuddy?.rootDirChanged?.on(() => {
        // Re-declare active fs subscriptions as well: the pe ids are stable but
        // the backend now resolves them to a different absolute directory.
        onReconnect();
        void mutate();
      }),
    [mutate]
  ...
  // resolve it against that entry's display path for the bundled path-based API.
  ...
  const handleOpenFile = async (peId: string, relativePath: string): Promise<void> => {
    try {
      const rootPath = data?.explorer.entries.find((entry) => entry.pe_id === peId)?.display_path;
      if (!rootPath) throw new Error(`Explorer root not found: ${peId}`);
      const { content, contentType, metadata } = await buildExplorerPreviewPayload(peId, relativePath, rootPath);
