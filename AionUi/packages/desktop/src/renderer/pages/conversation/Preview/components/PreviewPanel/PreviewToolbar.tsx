...
interface PreviewToolbarProps {
  ...
  /**
   * 下载为 PDF（仅 markdown 类型时用于弹出「PDF / 原文」选择）
   * Download as PDF (markdown only — shows the PDF/original picker)
   */
  onDownloadAsPdf?: () => void;

  /**
   * 下载为 Word（仅 markdown 类型）
   * Download as Word (markdown only)
   */
...
const PreviewToolbar: React.FC<PreviewToolbarProps> = ({
  ...
  /** 下载按钮：markdown 且有 PDF 能力时弹出「下载为 PDF / 下载原文」选择。 */
  const renderDownloadButton = () => {
    ...
    if (isMarkdown && onDownloadAsPdf) {
      return (
        <Dropdown
          droplist={
            <Menu
              onClickMenuItem={(key) => {
                if (key === 'pdf') onDownloadAsPdf();
                else if (key === 'word') onDownloadAsWord?.();
                else void onDownload();
              }}
            >
              <Menu.Item key='pdf'>{t('preview.downloadAsPdf')}</Menu.Item>
              {onDownloadAsWord && <Menu.Item key='word'>{t('preview.downloadAsWord')}</Menu.Item>}
              <Menu.Item key='original'>{t('preview.downloadOriginal')}</Menu.Item>
            </Menu>
          }
        >
          <div className={toolbarBtn} title={t('preview.downloadFile')}>
            {icon}
    ...
    <div className='flex items-center justify-between h-32px px-10px bg-bg-2 flex-shrink-0 border-b border-border-1 overflow-x-auto'>
      <div className='flex items-center justify-between gap-8px w-full' style={{ minWidth: 'max-content' }}>
        ...
        <div className='flex items-center h-full gap-8px'>
          ...
          {preferActionButtonsInFront && showDownload && renderDownloadButton()}
          {leftExtra}
        </div>
