...
export const useLocalFilePreview = (workspace?: string) => {
  ...
  return useCallback(
    async (file_path: string, reference?: LocalFileLinkReference) => {
      ...
      try {
        // The bundled runtime exposes the path-based file API. Metadata both
        // verifies existence and avoids the unavailable ChatFileRef endpoints.
        await ipcBridge.fs.getFileMetadata.invoke({ path: file_path, workspace });

        if (contentType === 'image') {
          content = (await ipcBridge.fs.getImageBase64.invoke({ path: file_path, workspace })) || '';
        } else if (contentType === 'pdf') {
          content = (await ipcBridge.fs.getImageBase64.invoke({ path: file_path, workspace })) || '';
        } else if (shouldReadPreviewContent(contentType)) {
          content = (await ipcBridge.fs.readFile.invoke({ path: file_path, workspace })) || '';
