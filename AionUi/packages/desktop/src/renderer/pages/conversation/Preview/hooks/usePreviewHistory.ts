...
export const usePreviewHistory = ({ activeTab, updateContent }: UsePreviewHistoryOptions): UsePreviewHistoryReturn => {
  ...
  // 按目标文件缓存历史列表：切回已看过的文件时直接读缓存，不重发请求、不闪烁 loading。
  const historyCacheRef = useRef<Map<string, PreviewSnapshotInfo[]>>(new Map());
  ...
  // useCache:true 时命中缓存则直接返回（切回已看过的文件不重发请求、不闪烁 loading）；
  // 手动刷新与保存快照后调用时不带该选项，强制重取并更新缓存。
  const refreshHistory = useCallback(
    async (options?: { useCache?: boolean }) => {
      ...
      const cacheKey = `${historyTarget.contentType}:${historyTarget.file_path ?? ''}`;
      const cached = historyCacheRef.current.get(cacheKey);
      if (options?.useCache && cached) {
        setHistoryVersions(cached);
      ...
      try {
        ...
        historyCacheRef.current.set(cacheKey, versions || []);
  ...
  useEffect(() => {
    void refreshHistory({ useCache: true }).catch((): void => undefined);
  }, [refreshHistory]);
