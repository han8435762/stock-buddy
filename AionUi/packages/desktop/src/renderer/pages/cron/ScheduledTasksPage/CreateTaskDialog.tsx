...
const CreateTaskDialog: React.FC<CreateTaskDialogProps> = ({
  ...
  // Manual task creation always executes with Claude Code; the assistant
  // selector is hidden in create mode (see render). Resolve the Claude Code
  // assistant by its ACP backend key so we don't hardcode a database id.
  const claudeCodeAssistant = useMemo(
    () => presetAssistants.find((assistant) => assistantRuntimeKey(assistant) === 'claude'),
    [presetAssistants]
  );
  ...
  // Both create and edit modes execute with Claude Code; the assistant
  // selector is hidden entirely. Keep selection in sync with the resolved
  // default instead of relying on a mounted Form.Item.
  useEffect(() => {
    if (!visible || !claudeCodeAssistant) return;
    setSelectedAssistantId(claudeCodeAssistant.id);
    form.setFieldsValue({ assistant: claudeCodeAssistant.id });
  }, [visible, claudeCodeAssistant, form]);
