...
const SystemModalContent: React.FC = () => {
  ...
  const [stockBuddyRootDir, setStockBuddyRootDir] = useState('');
  const [changingStockBuddyRootDir, setChangingStockBuddyRootDir] = useState(false);
  ...
  const handleChangeStockBuddyRootDir = async (): Promise<void> => {
    ...
    modal.confirm({
      ...
      onOk: async () => {
        setChangingStockBuddyRootDir(true);
        try {
          const updatedRootDir = await tryChangeStockBuddyDirectory(
            () => ipcBridge.stockbuddy.changeRootDir.invoke({ directory: nextRootDir }),
            (caughtError) => {
              Message.error(
                `${t('stockbuddy.settings.changeRootDirFailed')}: ${caughtError instanceof Error ? caughtError.message : String(caughtError)}`
              );
            }
          );
          if (!updatedRootDir) return;
          setStockBuddyRootDir(updatedRootDir);
          Message.success(t('stockbuddy.settings.changeRootDirSuccess'));
        } finally {
          setChangingStockBuddyRootDir(false);
        }
      },
    });
  };
        ...
        <div className='space-y-16px'>
          <div className='px-[12px] md:px-[32px] py-16px bg-2 rd-16px space-y-12px'>
            ...
            <Form form={form} layout='vertical' className='!mt-32px space-y-16px' onValuesChange={handleValuesChange}>
              ...
              {isDesktop && (
                <PreferenceRow
                  label={t('stockbuddy.settings.rootDir')}
                  description={t('stockbuddy.settings.changeRootDirHint')}
                >
                  <div className='flex flex-col items-end gap-4px max-w-420px'>
                    <div className='flex items-center gap-8px'>
                      <div className='text-12px text-t-secondary max-w-240px truncate' title={stockBuddyRootDir}>
                        {stockBuddyRootDir}
                      </div>
                      <Button
                        size='small'
                        loading={changingStockBuddyRootDir}
                        onClick={() => void handleChangeStockBuddyRootDir()}
                      >
                        {t('stockbuddy.settings.changeRootDir')}
                      </Button>
                    </div>
                    {changingStockBuddyRootDir && (
