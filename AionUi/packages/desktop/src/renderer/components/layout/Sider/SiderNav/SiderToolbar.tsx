...
interface SiderToolbarProps {
  ...
  /** Hide the secondary batch-management action for specialized entry points. */
  showBatchModeButton?: boolean;
  /** Optional i18n key for the primary action label; defaults to new-conversation copy. */
...
const SiderToolbar: React.FC<SiderToolbarProps> = ({
  ...
  onNewChat,
  onToggleBatchMode,
  showBatchModeButton = true,
  actionLabelKey,
}) => {
  const { t } = useTranslation();
  const actionLabel = actionLabelKey ? t(actionLabelKey) : t('conversation.welcome.newConversation');
    ...
    <div className='shrink-0 flex items-center gap-8px'>
      <Tooltip {...siderTooltipProps} content={actionLabel} position='right'>
        ...
        >
          ...
          <span className='collapsed-hidden text-t-primary text-14px font-[500] leading-24px'>{actionLabel}</span>
        </div>
      </Tooltip>
      {showBatchModeButton && (
        <Tooltip
        ...
        >
          <div
            data-testid='sider-batch-mode-toggle'
            className={classNames(
