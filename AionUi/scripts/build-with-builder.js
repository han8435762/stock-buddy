...
function resolveElectronBuilderCompression(level) {
  const numericLevel = Number.parseInt(level, 10);
  if (Number.isFinite(numericLevel) && numericLevel <= 1) return 'store';
  if (Number.isFinite(numericLevel) && numericLevel >= 9) return 'maximum';
  return 'normal';
}
...
try {
  ...
  // 5b. Prepare bundled portable Python (astock scripts run discover/prewarm
  // without a system Python). Staged under resources/bundled-python/ and packed
  // via extraResources, same pattern as bundled-aioncore.
  const { prepareBundledPython } = require('../packages/shared-scripts/src/prepare-bundled-python.js');
  prepareBundledPython({ projectRoot, platform: targetPlatform, arch: targetArch });
  ...
  // electron-builder accepts named compression modes. Keep the numeric env
  // variable as the public build-script switch and translate it explicitly.
  ...
  const builderCompression = resolveElectronBuilderCompression(process.env.ELECTRON_BUILDER_COMPRESSION_LEVEL);
  console.log(`📦 electron-builder compression mode: ${builderCompression}`);
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const aionRunning = isProcessRunningWindows('StockBuddy.exe');
      ...
      if (aionRunning || electronRunning) {
        ...
        killWindowsProcesses(['StockBuddy.exe', 'electron.exe']);
  ...
  const builderCommand = `bunx electron-builder --config packages/desktop/electron-builder.yml --config.compression=${builderCompression} ${builderArgs} ${archFlag} ${nsisInclude} ${publishArg}`;
  try {
    buildWithDmgRetry(builderCommand, targetArch);
  } catch (error) {
    const winExePath = path.join(outDir, 'win-unpacked', 'StockBuddy.exe');
    ...
    console.log('⚠️  Windows local build failed after StockBuddy.exe was produced.');
    if (firstError) {
