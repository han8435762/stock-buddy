...
function verifyBundledPython(resourcesDir, electronPlatformName, targetArch) {
  const pythonDir = path.join(resourcesDir, 'bundled-python', `${electronPlatformName}-${targetArch}`);
  const exe =
    electronPlatformName === 'win32' ? path.join(pythonDir, 'python.exe') : path.join(pythonDir, 'bin', 'python3');
  if (!fs.existsSync(exe)) {
    throw new Error(`Packaged app is missing bundled python runtime: ${exe}`);
  }
  console.log(`   ✓ Bundled python verified for ${electronPlatformName}-${targetArch}`);
}
...
module.exports = async function afterPack(context) {
  ...
  if (fs.existsSync(resourcesDir)) {
    ...
    verifyBundledPython(resourcesDir, electronPlatformName, targetArch);
  } else {
