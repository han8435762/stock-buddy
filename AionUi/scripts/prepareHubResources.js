...
function isHubCacheComplete(tag, hubDir = HUB_DIR) {
  try {
    const indexPath = path.join(hubDir, 'index.json');
    const manifestPath = path.join(hubDir, 'manifest.json');
    if (!fs.existsSync(indexPath) || fs.statSync(indexPath).size === 0 || !fs.existsSync(manifestPath)) {
      return false;
    }
    ...
    if (
      ...
      manifest.extensions.length !== manifest.total
    ) {
      return false;
    }

    return manifest.extensions.every((entry) => {
      if (!entry || typeof entry.file !== 'string' || path.basename(entry.file) !== entry.file) return false;
      const filePath = path.join(hubDir, entry.file);
...
async function prepareHubResources() {
  ...
  if (isHubCacheComplete(tag)) {
    console.log(`[hub] Complete bundle for ${tag} found; using cached resources`);
  ...
  const bundleTotal = names.filter((name) => Boolean(extensions[name]?.dist?.tarball)).length;
  ...
  return { skipped: false, count: results.length, total: bundleTotal };
}
...
module.exports = { isHubCacheComplete, prepareHubResources };
