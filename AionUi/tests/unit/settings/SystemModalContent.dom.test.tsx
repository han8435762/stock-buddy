...
vi.mock('@/common', () => ({
  ipcBridge: {
    ...
    stockbuddy: {
      getRootDir: { invoke: vi.fn(() => Promise.resolve('/Users/hanyang/StockBuddy')) },
      changeRootDir: { invoke: vi.fn(() => Promise.resolve('/new-root')) },
    },
    shell: {
...
describe('SystemModalContent directory settings', () => {
  ...
  it('opens the StockBuddy directory picker without scanning the current library first', async () => {
    const user = userEvent.setup();
    showOpenMock.mockResolvedValueOnce([]);
    renderContent();

    await user.click(await screen.findByRole('button', { name: 'stockbuddy.settings.changeRootDir' }));

    expect(showOpenMock).toHaveBeenCalledWith({ properties: ['openDirectory', 'createDirectory'] });
  });
