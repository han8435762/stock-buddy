...
describe('AboutModalContent update ready state', () => {
  ...
  it('opens StockBuddy repository links from About', async () => {
    render(<AboutModalContent />);

    fireEvent.click(screen.getByText('settings.updateLog'));
    await waitFor(() => {
      expect(mocks.openExternalUrlMock).toHaveBeenCalledWith('https://github.com/han8435762/stock-buddy/releases');
    });

    expect(mocks.openExternalUrlMock).not.toHaveBeenCalledWith(expect.stringContaining('iOfficeAI'));
    expect(mocks.openExternalUrlMock).not.toHaveBeenCalledWith(expect.stringContaining('aionui.com'));
  });
});
