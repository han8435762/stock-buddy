...
describe('MessageText attachment paths', () => {
  beforeEach(() => {
    ...
    vi.mocked(ipcBridge.fs.getFileMetadata.invoke).mockResolvedValue(fileMetadata('/x'));
  ...
  it('opens a missing-file preview when a local markdown link no longer exists', async () => {
    ...
    vi.mocked(ipcBridge.fs.getFileMetadata.invoke).mockRejectedValue(new Error('not found'));
    localFileLinkMocks.payload = {
      path: '/missing/report.xlsx',
      reference: {
  ...
  it('opens office and pdf local markdown links without reading file content', async () => {
    ...
    vi.mocked(ipcBridge.fs.getImageBase64.invoke).mockResolvedValue('data:application/pdf;base64,PDF');
    ...
    expect(ipcBridge.fs.getImageBase64.invoke).toHaveBeenCalledWith({ path: filePath, workspace: '/workspace/demo' });
  });
