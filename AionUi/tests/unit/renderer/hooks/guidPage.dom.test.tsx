  ...
  useTranslation: () => ({
    t: (
      ...
      const messages: Record<string, string> = {
        "stockbuddy.newResearch.promptResearchCompany":
          "使用`yanbao`skill 生成这个公司的研究报告",
        "stockbuddy.newResearch.promptRecentAnnouncements":
          "总结一下最近三个月的公告",
      };
...
vi.mock("@/renderer/pages/guid/hooks/useGuidAssistantSelection", () => ({
  useGuidAssistantSelection: (...args: unknown[]) =>
    useGuidAssistantSelectionMock(...args),
...
vi.mock("@/renderer/pages/guid/utils/assistantDefaults", () => ({
  resolveGuidAssistantDefaults: (...args: unknown[]) =>
    resolveGuidAssistantDefaultsMock(...args),
}));
...
describe("GuidPage", () => {
  ...
  it("appends a draft-preserving prefill without clearing attachments or workspace", () => {
    ...
    expect(inputUpdater("Existing Guid draft")).toBe(
      "Existing Guid draft\nCreate with /cron in AionUi",
    );
    expect(guidInputMock.setFiles).not.toHaveBeenCalled();
    expect(guidInputMock.setDir).not.toHaveBeenCalled();
    expect(capturedGuidInputCardProps.at(-1)?.focusRequestKey).toBe(
      "guid-location",
    );
    ...
    expect(guidInputMock.setInput).toHaveBeenCalledWith("");
    expect(
      guidInputMock.setInput.mock.calls.some(
  ...
  it("keeps a generic conversation heading and omits assistant-detail chrome on the home page", () => {
    ...
    expect(
      screen.queryByLabelText("Assistant Details"),
    ...
    expect(
      screen.queryByText("conversation.welcome.quickActionFeedback"),
    ).not.toBeInTheDocument();
    const latestAssistantSelectionAreaProps =
      capturedAssistantSelectionAreaProps.at(-1);
    ...
    expect(latestAssistantSelectionAreaProps).not.toHaveProperty(
      "is_presetAgent",
    );
    expect(latestAssistantSelectionAreaProps).not.toHaveProperty(
      "selectedAgentInfo",
    ...
    expect(latestGuidActionRowProps?.inlineCapabilities).toBe(false);
  ...
  it("applies the company research layout, prompts, full-auto permission, and all-skill defaults", async () => {
    locationMock.state = {
    ...
    expect(
      screen.queryByTestId("assistant-selection-area"),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("button", {
        name: "使用`yanbao`skill 生成这个公司的研究报告",
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "总结一下最近三个月的公告" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "guid.defaultPrompts.understand" }),
    ).not.toBeInTheDocument();

    await waitFor(() => {
      expect(agentSelectionMock.setSelectedMode).toHaveBeenCalledWith(
        "bypassPermissions",
        {
          persistPreference: false,
        },
      );
      expect(capturedGuidActionRowProps.at(-1)).toMatchObject({
        inlineCapabilities: true,
        disabledBuiltinSkills: [],
        enabledSkills: ["financial-review"],
      });
      expect(capturedGuidSendDeps.at(-1)).toMatchObject({
        guidDisabledBuiltinSkills: [],
        guidEnabledSkills: ["financial-review"],
      });
    });
  });
  ...
  it("falls back to default instruction prompts when the selected assistant has no recommendations", () => {
    ...
    expect(
      screen.getByRole("button", { name: "guid.defaultPrompts.understand" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "guid.defaultPrompts.cleanup" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "guid.defaultPrompts.create" }),
    ).toBeInTheDocument();
  });
  ...
  it("does not reapply assistant default model over a guid-page model selection", async () => {
    ...
    await vi.waitFor(() => {
      expect(agentSelectionMock.setSelectedAcpModel).toHaveBeenCalledWith(
        "default",
        { persistPreference: false },
      );
    });
    ...
    const modelSelectorNode =
      latestActionRowProps?.modelSelectorNode as React.ReactElement<{
    ...
    expect(agentSelectionMock.setSelectedAcpModel).not.toHaveBeenCalledWith(
      "default",
      {
        persistPreference: false,
      },
...
describe("GuidInputCard prefill focus", () => {
  it("focuses a draft-preserving prefill on desktop", async () => {
    ...
    const { rerender } = render(
      ...
      >
        <GuidInputCard
          {...guidInputCardProps}
          input={originalDraft}
          focusRequestKey="desktop-prefill"
        />
      </LayoutContext.Provider>,
    );
    ...
    await waitFor(() => expect(textarea).toHaveFocus());
    expect((textarea as HTMLTextAreaElement).selectionStart).toBe(
      guidInputCardProps.input.length,
    );
    ...
    rerender(
      ...
      >
        <GuidInputCard
          {...guidInputCardProps}
          input={`${guidInputCardProps.input}!`}
        />
      </LayoutContext.Provider>,
    );
    expect(outsideTarget).toHaveFocus();
  ...
  it("does not force focus for a mobile prefill", async () => {
    ...
    render(
      ...
      >
        <GuidInputCard
          {...guidInputCardProps}
          focusRequestKey="mobile-prefill"
        />
      </LayoutContext.Provider>,
    );
