...
import {
  ...
  pickPreferredAssistantSelectionKey,
...
describe('guid assistant selection helpers', () => {
  ...
  it('prefers the Claude Code assistant for company research', () => {
    expect(pickPreferredAssistantSelectionKey(assistants, 'claude')).toBe('builtin-writer');
  });

  it('falls back to the normal default when the preferred backend is unavailable', () => {
    expect(pickPreferredAssistantSelectionKey(assistants, 'codex')).toBe('bare-aionrs');
  });
