...
vi.mock('@/common', () => ({
  ipcBridge: {
    ...
    fs: {
      ...
      readFile: { invoke: (p: unknown) => readFile(p) },
      getImageBase64: { invoke: (p: unknown) => getImageBase64(p) },
    },
...
beforeEach(() => {
  ...
  readFile.mockReset().mockResolvedValue('hello');
  getImageBase64.mockReset().mockResolvedValue('');
...
describe('ExplorerContainer attach/remove', () => {
  ...
  it('opens a selected file inside the migrated project root', async () => {
    renderIt();
    await screen.findByTestId('roots');
    fireEvent.click(screen.getByTestId('do-open'));
    await waitFor(() =>
      expect(readFile).toHaveBeenCalledWith({
        path: '/x/docs/readme.md',
        workspace: '/x',
      })
    );
    ...
    expect(metadata).toMatchObject({ file_path: '/x/docs/readme.md', workspace: '/x' });
  });
