...
let rootDirChangedHandler: (() => void) | null = null;
vi.mock('@/common', () => ({
  ipcBridge: {
    ...
    stockbuddy: {
      rootDirChanged: {
        on: (handler: () => void) => {
          rootDirChangedHandler = handler;
          return () => {
            rootDirChangedHandler = null;
          };
        },
      },
    },
...
describe('ExplorerContainer data integration', () => {
  ...
  it('refetches project roots when the StockBuddy directory changes', async () => {
    projectGet.mockResolvedValueOnce(detail([entry({ display_name: 'Old Root' })]));
    projectGet.mockResolvedValueOnce(detail([entry({ display_name: 'New Root', display_path: '/new/root' })]));
    renderContainer('p1');

    expect(await screen.findByText('Old Root')).toBeInTheDocument();
    rootDirChangedHandler?.();
