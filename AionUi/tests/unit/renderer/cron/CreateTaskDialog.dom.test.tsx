...
describe('CreateTaskDialog', () => {
  ...
  it('hides the assistant selector and pins Claude Code in edit mode', async () => {
    const user = userEvent.setup();
    ...
    await waitFor(() => expect(ipcBridge.cron.updateJob.invoke).toHaveBeenCalledTimes(1));
    expect(resolveCronAgentConfig).toHaveBeenCalledTimes(1);
    expect(vi.mocked(resolveCronAgentConfig).mock.calls[0][0]).toMatchObject({
      agentValue: 'claude-1',
    });
  });

  it('never shows an assistant name when editing a task without a stored assistant id', async () => {
  ...
  it('hides the assistant selector when editing an ongoing conversation task', async () => {
    render(<CreateTaskDialog visible onClose={() => {}} editJob={ongoingConversationJob()} />);

    expect(await screen.findByText('cron.page.form.existingConversation')).toBeInTheDocument();
    expect(screen.queryByTestId('cron-assistant-select')).not.toBeInTheDocument();
    expect(screen.queryByText('cron.page.form.assistantLockedExistingConversation')).not.toBeInTheDocument();
  });

  it('keeps the assistant selector hidden when an ongoing conversation task is temporarily switched to new conversation', async () => {
    const user = userEvent.setup();
  ...
  it('locks execution mode and hides the assistant selector when editing a team-owned task', async () => {
    const user = userEvent.setup();
  ...
  it('defaults to Claude Code and hides the assistant selector when manually creating a task', async () => {
    const user = userEvent.setup();

    render(<CreateTaskDialog visible onClose={() => {}} />);

    // Create mode always executes with Claude Code — the selector is hidden.
    expect(screen.queryByTestId('cron-assistant-select')).not.toBeInTheDocument();
