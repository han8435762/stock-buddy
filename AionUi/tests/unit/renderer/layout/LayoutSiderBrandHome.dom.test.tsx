...
vi.mock('@/renderer/components/settings/ClaudeCodeGuideDialog', () => ({ default: () => null }));
...
describe('Layout sider brand Home button', () => {
  ...
  it('falls back to the companies page in a settings route when no path is recorded', () => {
    currentPathname = '/settings/system';
    renderLayout();

    fireEvent.click(screen.getByLabelText(BACK_KEY));
    expect(navigate).toHaveBeenCalledWith('/stockbuddy/companies');
  });

  it('falls back to the companies page when the recorded path is itself a settings path', () => {
    currentPathname = '/settings/about';
  ...
  it('renders the wordmark as a non-actionable element in a non-settings route', () => {
    ...
    const wordmark = screen.getByText('StockBuddy');
  ...
  it('does not navigate when the wordmark is clicked in a non-settings route', () => {
    ...
    fireEvent.click(screen.getByText('StockBuddy'));
    expect(navigate).not.toHaveBeenCalled();
  });
