...
vi.mock('@/common', () => ({
  ipcBridge: {
    fs: {
      ...
      writeSkillFile: { invoke: mocks.writeSkillFile },
      getSkillDefaultFile: { invoke: mocks.getSkillDefaultFile },
    },
  },
}));
...
describe('SkillFileBrowser', () => {
  ...
  it('renders markdown files in preview mode with source toggle, save and restore', async () => {
  ...
  it('renders non-markdown files as editable with save and restore', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('code-editor')).toHaveAttribute('data-read-only', 'false'));
    expect(screen.getByText('Save')).toBeInTheDocument();
    expect(screen.getByText('Restore default')).toBeInTheDocument();
  });
