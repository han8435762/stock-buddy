...
describe('updateBridge GitHub release URLs', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('keeps release assets on the requested GitHub repository', async () => {
  ...
  it('does not contact the legacy CDN during version detection', async () => {
    const fetchMock = stubCdnAndGitHubFetch();
    ...
    try {
      ...
      const result = await handler({ repo: 'iOfficeAI/AionUi' });
      expect(result.success).toBe(true);
      expect(fetchMock.mock.calls.every(([input]) => !String(input).startsWith('https://static.aionui.com'))).toBe(true);
    } finally {
      vi.unstubAllGlobals();
    }
  });
