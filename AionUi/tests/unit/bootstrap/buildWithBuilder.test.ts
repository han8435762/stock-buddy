...
function resolveAppBuilderInstallUtil(): string {
  ...
  const packageStoreDirs = [resolve(repoRoot, 'node_modules/.bun'), resolve(repoRoot, 'node_modules/.pnpm')];
  const appBuilderDir = packageStoreDirs
    .filter((directory) => existsSync(directory))
    .flatMap((directory) =>
      readdirSync(directory, { withFileTypes: true })
        .filter((entry) => entry.isDirectory() && entry.name.startsWith('app-builder-lib@'))
        .map((entry) => resolve(directory, entry.name, 'node_modules/app-builder-lib'))
    )
  ...
  it.each([
    {
      ...
      expectedArch: 'arm64',
      expectedPythonPlatform: 'win32',
    },
    {
      args: ['auto', '--mac', '--x64'],
      expectedArch: 'x64',
      expectedPythonPlatform: 'darwin',
    },
  ])('prepares $expectedPythonPlatform-$expectedArch runtimes', ({ args, expectedArch, expectedPythonPlatform }) => {
...
Module._load = function patchedLoad(request, parent, isMain) {
  ...
  if (request.endsWith('packages/shared-scripts/src/prepare-bundled-python.js')) {
    return { prepareBundledPython: recordPrepareCall };
  }
...
`,
    ...
    try {
      ...
      expect(calls).toContainEqual(expect.objectContaining({ arch: expectedArch, platform: expectedPythonPlatform }));
    } finally {
      rmSync(outDir, { recursive: true, force: true });
      if (movedExistingOut) {
        renameSync(backupOutDir, outDir);
