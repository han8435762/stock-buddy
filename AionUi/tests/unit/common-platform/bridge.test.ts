...
describe('local bridge', () => {
  ...
  it('logs rejected providers and emits an error callback instead of a success one', async () => {
    ...
    // The error callback lets the invoke reject instead of hanging forever.
    expect(outbound.some(({ name }) => name === 'subscribe.error-test.failurerequest-1')).toBe(true);
  });

  it('rejects the invoke when the provider rejects, across a JSON-serializing transport', async () => {
    const { bridge } = await loadSerializingBridge();
    vi.spyOn(console, 'error').mockImplementation(() => {});
    const endpoint = bridge.buildProvider<string, void>('test.reject-async');
    endpoint.provider(() => Promise.reject(new Error('async exploded')));

    await expect(endpoint.invoke()).rejects.toThrow('async exploded');
  });

  it('rejects the invoke when the provider throws synchronously', async () => {
    const { bridge } = await loadSerializingBridge();
    vi.spyOn(console, 'error').mockImplementation(() => {});
    const endpoint = bridge.buildProvider<string, void>('test.reject-sync');
    endpoint.provider(() => {
      throw new Error('sync exploded');
    });

    await expect(endpoint.invoke()).rejects.toThrow('sync exploded');
  });
});
