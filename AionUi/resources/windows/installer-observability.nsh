...
!define AIONUI_APP_EXECUTABLE_FILENAME "StockBuddy.exe"
...
!macro AIONUI_LOG_EXTRACT_RESULT _METHOD
  ${IfNot} ${FileExists} "$INSTDIR\StockBuddy.exe"
    !insertmacro AIONUI_FAIL_UX \
      "${AIONUI_E_EXTRACT_FAILED}" \
      "event=extract result=fail method=${_METHOD} missing=StockBuddy.exe" \
      ...
      "extract result=fail method=${_METHOD} missing=StockBuddy.exe instDir=$INSTDIR" \
      "extract result=fail method=${_METHOD} missing=StockBuddy.exe instDir=$INSTDIR"
  ${Else}
