...
AI agents must not push unless explicitly asked:

```bash
git push                          # push the current branch to origin
git push -u origin feat/branch    # push a new branch and set its upstream
```

Before pushing, run the checks under [During Development](#during-development) to keep the branch clean.
