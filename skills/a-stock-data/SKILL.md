...
### Python 运行时检查与中国大陆安装兜底（必须执行）

运行任何 Python 代码前，按以下顺序处理：

1. 优先使用应用注入的 `AIONUI_PYTHON_EXECUTABLE`；若该文件存在，直接使用它。
2. 否则依次检测 `python`、`python3`、Windows 的 `py -3`，并实际执行 `--version` 验活。不要只扫描常见安装目录，也不要因为 `where python` 无结果就断言没有 Python。
3. 只有以上解释器均不可用时才安装 Python。Windows 必须优先从中国大陆镜像下载固定版本安装包，静默按当前用户安装，并在执行前验证微软 Authenticode 签名；安装完成后重新检测。
4. 安装 Python 包必须通过已选定的解释器执行 `-m pip`，并使用中国大陆 PyPI 镜像。不要使用裸 `pip`，避免装到另一个解释器。

Windows PowerShell 兜底示例（x64 / ARM64）：

```powershell
function Resolve-WorkingPython {
  $candidates = @(
    $env:AIONUI_PYTHON_EXECUTABLE,
    (Get-Command python -ErrorAction SilentlyContinue).Source,
    (Get-Command python3 -ErrorAction SilentlyContinue).Source
  ) | Where-Object { $_ }
  foreach ($candidate in $candidates) {
    $resolved = (& $candidate -c 'import sys; print(sys.executable)' 2>$null | Select-Object -First 1)
    if ($LASTEXITCODE -eq 0 -and $resolved -and (Test-Path $resolved)) { return $resolved }
  }
...
if (-not $python) {
  ...
  Invoke-WebRequest `
    -Uri "https://mirrors.aliyun.com/python-release/windows/$installerName" `
    -OutFile $installer `
    -TimeoutSec 120
  if ((Get-AuthenticodeSignature $installer).Status -ne 'Valid') {
    throw 'Python 安装包签名校验失败，已中止安装。'
  }
...
if (-not $python) { throw 'Python 安装后仍不可用。' }
& $python --version
& $python -m pip install -i https://pypi.tuna.tsinghua.edu.cn/simple --timeout 120 mootdx requests pandas stockstats
```

> 自动安装属于联网且会修改用户环境的兜底动作：仅在确认内置及系统解释器均不可用时执行。阿里云镜像用于 Python Windows 安装包；清华 PyPI 镜像用于 Python 包，二者不可混用。
...
> ⛔ **已死透别用**（2026-07 实测）：网易财经(126.net 整站下线)、和讯、凤凰行情、腾讯资金流(ff\_ 已死)、雪球免登录深度数据(需 token)。mootdx **库**已烂尾(2024 停更)但**通达信 TCP 协议本身照常**——继续用，装不上就用 `tdx_client()`。
>
...
# 3. 按「Python 运行时检查与中国大陆安装兜底」选择解释器，再从国内镜像安装依赖
python -m pip install -i https://pypi.tuna.tsinghua.edu.cn/simple --timeout 120 mootdx requests pandas stockstats
